...
def extract_params(infile,
    ...
    try: 
        with open(infile, mode='rb') as file:
            ...
            while True:
                data = file.read(12)  # Reading 16 bytes at a time (4 bytes for each interval)
    ...
    d0 = np.array(arrays[1]) # A0 digital value
    d1 = np.array(arrays[2]) # A1 digital value
    ...
    return t1,v0,v1,inter_sample_delay,inter_average_delay,samples_averaged
...
def quickLook(infile, 
    ...
    t_1,v_0,v_1,inter_sampleDelay,inter_averageDelay,samples_averaged = extract_params(infile, 
                                                                                          samples_averaged = s,
    ...
    axs[2].set_title(f'Histogram of Gaps: {num_long_gaps} long gaps, median: {median_long_gap} ms, Dead Time {round(dead_time,3)}%')
    axs[2].grid()
...
def analyze(infile, 
    ...
    t_1,_,_,inter_sampleDelay,inter_averageDelay,samplesAveraged = extract_params(infile, 
                                                                                      samples_averaged=s,
                                                                                      inter_sample_delay=isd,
                                                                                      header_infile=h )
    ...
    gap_qualifier = dt_med*qualifier_multiplier
    ...
    file_t_calc = len(dt[long_gap])*np.median(dt[long_gap]) + dt_med*len(small_gap)
    # Calculation:
    ...
    freq_averaging = samplesAveraged/dt_med # dt_med = approx actual INTRA
    ...
    if prints == True: 
        ...
        print(f"\ttot_samples = {len(t_mid)}, INTA = {dt_med}")
