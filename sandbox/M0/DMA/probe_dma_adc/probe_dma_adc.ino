...
void adc_init(){
  ...
  ADC->CTRLB.reg = ADC_CTRLB_PRESCALER_DIV512  | ADC_CTRLB_FREERUN | ADC_CTRLB_RESSEL_12BIT;
  ADCsync();
...
void setup(){
	...
	// analogWrite(A0,511);   // test with DAC
	adc_init();
	dma_init();
}
...
void loop() {
  ...
  while(!dmadone && micros() - t < 2*HWORDS + 13 ); // required to fix stalls 
	t = micros() - t;
	Serial.print(t/HWORDS);  Serial.print(" us per conversion ");
	Serial.println(adcbuf[0]*(3.3/4096),4);
	delay(500); // must include some delay or device will stall but  haven't determined mimimum 

  /* Note: https://forum.arduino.cc/t/using-adc-with-dma/498085/9
  n8k: I was able to resolve my issues by raising the prescaler and 
  sample control by a small amount and double buffering the adc 
  data I was processing. I also switched to using Adafruit's 
  ZeroDMA since it's bundled with my board's libraries 
  */
}